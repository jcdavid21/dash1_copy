<?php 
    $itemModalSql = "SELECT tc.*, ti.*, ts.*, tp.*, tz.pizzaPrice FROM `viewcart` tc INNER JOIN orderitems ti ON tc.cartitemId = ti.cartitemId INNER JOIN order_status ts ON tc.status_id = ts.status_id INNER JOIN payment_mode tp ON ti.payment_id = tp.payment_id INNER JOIN pizza tz ON tc.pizzaId = tz.pizzaId WHERE ti.status_id IN(2, 3, 4, 5, 6) ORDER BY ti.order_id DESC";
                ...
                <div class="container">
                    <div class="row">
                        ...
                        <div class="table-responsive">
                            ...
                            <tbody>
                                <?php
                                     $pizzaId = $itemModalRow['pizzaId'];
                                     $itemQuantity = $itemModalRow['itemQuantity'];
